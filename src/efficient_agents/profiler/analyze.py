...
class Analyzer:
    ...
    def process_glances_log(self):
        data = [{
            ...
            "smctemp_cpu": next((s["value"] for s in l.get("sensors", []) if s["label"] == "smctemp"), 0),
            "battery_percent": next((s["value"] for s in l.get("sensors", []) if s["label"] == "Battery"), 0),
            "battery_discharge": next((-min(s["value"], 0) for s in l.get("sensors", []) if s["label"] == "Battery Current"), 0),
            "battery_temp": next(((s["value"] / 10 - 273.15) for s in l.get("sensors", []) if s["label"] == "Battery Temperature"), 0),
            "fan_max_speed": max((s["value"] for s in l.get("sensors", []) if s["label"].startswith("Fan")), default=0),
